...
class ContAwareTimeSeriesImp(object):
    def __init__(self, var_names,
        ...
        print(self.out_path)
        # load data
        self.train_set, self.valid_set = load_data(Path(train_data_path), reload_raw=force_reload_raw, shuffle=False,
    ...
    def fit(self, epochs=300, batch_size=64, eval_batch_size=64, eval_epoch=1, record_imp_epoch=50):
        ...
        for epoch in range(epochs):
            ...
            if epoch+1 % 100 == 0:
                print('Change lr x 0.1.')
                optimizer.param_groups[0]["lr"] = optimizer.param_groups[0]["lr"] * 0.1
                optimizer.param_groups[1]["lr"] = optimizer.param_groups[0]["lr"] * 0.1
        ...
        torch.save(self.model.state_dict(), self.out_path / 'imp_model.pt')
    ...
    def evaluate(self, data_iter, print_scores=True):
        ...
        for idx, data in enumerate(data_iter):
            ...
            # print(data['pids'])
            # print(eval_masks.sum(dim=[0, 1]).tolist())
            # if 0.0 in eval_masks.sum(dim=[0, 1]).tolist():
            #     for j in range(eval_masks.shape[0]):
            #         for h in range(eval_masks.shape[1]):
            #             print(eval_masks[j][h])
...
if __name__ == '__main__':
    ...
    var_names = [	'EtCO2', 'BaseExcess', 'HCO3', 'pH', 'PaCO2', 'SaO2', 'AST', 'BUN', 'Alkalinephos', 'Calcium', 'Chloride',
